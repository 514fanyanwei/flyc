...
public final class ThreadPool {
	...
	private static Log taskLogger = LogFactory.getLog("TaskLogger");
	//private static boolean debug = taskLogger.isDebugEnabled();
	private static boolean debug = taskLogger.isInfoEnabled();//isDebugEnabled();
    ...
    /* 默认池中线程数 */
    public static int worker_num = 100;
    /* 已经处理的任务数 */
    ...
    private ThreadPool(int pool_worker_num) {
    	logger.debug("初始化报文处理线程池,大小:"+pool_worker_num);
    ...
    */
    public void batchAddTask(Task[] taskes) {
        ...
        for (int i = 0; i < taskes.length; i++) {
           ...
           logger.debug("Submit Task<" + taskes[i].getTaskId() + ">: "                + taskes[i].info());
        }
    }
    /**
    * 线程池信息
            ...
            while (isRunning) {
                ...
                if (r != null) {
                    ...
                    try {
                        ...
                        //taskLogger.debug(ThreadPool.getInstance(3).getInfo());
                    } catch (Exception e) {
                        e.printStackTrace();
                        logger.error(e);
                    }
