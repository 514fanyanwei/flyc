...
import cc.gps.parse.lzbus.LZBus0x55;

public class LZBusProcess0x55 extends LZBusProcessReceive{
	private static final Log log = LogFactory.getLog(LZBusProcess0x55.class);
	
	@Override
	public byte[] processMessageBody(JTReceiveData inData) {
		if(log.isDebugEnabled())
			log.info("处理来自"+inData.head.phone+"的"+Recourse.getOrder("lzbus0055")+":-------------");
		String body=inData.body;  //body为序列号后的实际内容
		log.info("应答内容为"+body);
		LZBus0x55 lz=new LZBus0x55(inData.bb,inData.head.len);
		
		//log.info(body);
		//向终端回送1个应答
		inData.head.messageID=0x55;
