...
public class LZBusProcess0x53 extends LZBusProcess0x41{
	...
	public byte[] processMessageBody(JTReceiveData inData) {
		if(log.isDebugEnabled())
			log.info("处理来自"+inData.head.phone+"的"+Recourse.getOrder("lzbus0053")+":-------------");
		
		JT0x0200 td=processData(inData);
		String body = inData.body;
		LZBusOverSpeedTime overTime=new LZBusOverSpeedTime();
		//超速时长
		overTime.time=Integer.parseInt(Ecode.HEX2DEC(body.substring(70,74)));
		...
		overTime.stationID=Integer.parseInt(before);//String extID=Integer.parseInt(before);
		
		
		//站间里程
		mile=Ecode.HEX2DEC(body.substring(64,68))+"."+Ecode.HEX2DEC(body.substring(68,70));//站间里程
		overTime.sInterval=Double.parseDouble(mile);
