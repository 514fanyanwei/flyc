...
public class LZBusProcessReceive implements IProcessReceive{
	private static final Log log = LogFactory.getLog(LZBusProcessReceive.class);
	...
	public final SendData processData(ByteBuf bb,ChannelHandlerContext ctx) {
		...
		//排除0xff
		if(bb.getUnsignedByte(5)==0xff){
			log.info("receive 0xff");
			//int mid=bb.getUnsignedByte(11);
			ByteBuf savebb=bb.duplicate();
			/*
			while (savebb.isReadable()) {
	    		byte b=savebb.readByte();
	    		log.info(Ecode.DEC2HEX(b,2));
	    		
	    	}log.info();*/
	    	
	    	//savebb=bb.duplicate(); //记录原始(包含ff)
	    	bb.skipBytes(6);
			start=6;
			/*
			savebb=bb.duplicate();
		...
		//写数据表GPSPacketLog,保存收到的原始报文  2014-6-12
		...
		Global.add("LZBUS", "R", "00"+str.substring(10,12), clientID, str.toString());
		...
		for(Iterator<JTReceiveData> it=datas.iterator();it.hasNext();){
			...
			if((jtrdata.head.messageID==0x75)||(jtrdata.head.messageID==0x45)){
				if(jtrdata.head.messageID==0x75){
					//现将clientID设为sim
			...
			LZBusProcessReceive process=LZBusProcessReceiveBodyFactory.createProcess(jtrdata.head.messageID);
			/*
	...
	final protected void processAnswer(int messageID,int serialID,int result,String clientID){
		...
		//log.debug("应发送待回应指令数量:"+Global.orderSended.size());
	...
	public final List<JTReceiveData> processMessageHead(ByteBuf bb,String clientID) {
		...
		head.messageID=bb.getUnsignedByte(start+5); //命令ID
		head.phone=clientID;
