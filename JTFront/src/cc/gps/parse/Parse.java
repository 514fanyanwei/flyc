...
public class Parse {
  ...
  //随机生成用16进制表示的4位长字符串
	...
	{
		log.info(Parse.A2S("303132"));
		log.info(Parse.S2A("0123"));
		log.info(Parse.binaryString2hexString("0000001000001001"));
		...
		log.info(Parse.HEX2DEC("012A"));
		
		log.info(Parse.DEC2HEX(-128));
		String s=Parse.DEC2HEX(3);
		while(s.length()<4)
			s="0"+s;
		log.info(Parse.HexString2ByteArray(s)[0]+"  "+Parse.HexString2ByteArray(s)[1]);
		byte[] bs={0x02,0x1a,0x03,0x56,0x1f,0x3d,0x36,0x31,0x37,0x36,};
		byte[] bs2={3,-1,18};
		log.info(Parse.ByteArray2HexString(bs, 0, 3));
		log.info(Parse.ByteArray2HexString(bs2,-1,-1));
		
		log.info(Byte16ArrayDecString(bs,0,3));
		
		log.info((-1)&0xff);
		byte[] b=Parse.HexString2ByteArray("8001");
		log.info(b[0]+"   "+(b[1]));
		
		
		b=createOrderID_old();
		log.info((b[0])+"   "+(b[1]));
		log.info(Parse.S2A("请求退出营运"));//c7ebc7f3cdcbb3f6d3aad4cb
	}
}
