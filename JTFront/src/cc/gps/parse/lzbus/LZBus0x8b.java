...
public class LZBus0x8b {
	private static final Log log = LogFactory.getLog(LZBus0x8b.class);
	...
	//skip(5);
	public Segment<String> ud;  //上、下行标志		0X01:上行  0X02:下行
	public Segment<String> se;  //发车或进入终点站	0X01:发车  0X02:进入终点
	
	private int s=0; //开始位置  //测试时改为6
	
	
	public LZBus0x8b(ByteBuf bb){
		bb.skipBytes(s);
		...
		bb.skipBytes(5);
		ud=new Segment<String>(bb,1,new StringParse());
		se=new Segment<String>(bb,1,new StringParse());
	}
	...
	public LZBusDriverRequest convert(){
		...
		if(se.value.equals("01")){
			driver.requestType="转入营运";
